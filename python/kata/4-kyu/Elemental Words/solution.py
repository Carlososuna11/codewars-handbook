ELEMENTS =  {
                ...
                'Rg': 'Roentgenium', 'Cn': 'Copernicium', 'Uut': 'Ununtrium', 'Fl': 'Flerovium', 
                'Uup': 'Ununpentium', 'Lv': 'Livermorium', 'Uus': 'Ununseptium', 'Uuo': 'Ununoctium'
                }
...
def elemental_forms(word):
    ...
    solutions = []
    while len(stack) > 0:
        solution,word = stack.pop()
        if len(word)==0:
            if solution not in solutions:
                solutions.append(solution)
        else:
            for i in range(3):
                if word[:i+1].capitalize() in ELEMENTS:
                    solution.append(f'{ELEMENTS[word[:i+1].capitalize()]} ({word[:i+1].capitalize()})')
                    stack.append((solution[:],word[i+1:]))
                    solution.pop()
    return solutions